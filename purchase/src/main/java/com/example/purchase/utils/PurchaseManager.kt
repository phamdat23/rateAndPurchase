...
object PurchaseManager {
    ...
    fun queryDetailPurchase(
        ...
        mBillingClient =
            BillingClient.newBuilder(context).enablePendingPurchases(pendingParams)
                .setListener { p0, p1 ->
    ...
    fun checkTimeIap(context: Context): Boolean {
        ...
        if (isIapTest) {
            ...
            return if (getCurrentKeyIap(context).endsWith("0.3")) {
                ...
                TimeUnit.MILLISECONDS.toMinutes(currentTime - timeBought) <= 3
            } else if (getCurrentKeyIap(context).endsWith("2")) {
                val check = TimeUnit.MILLISECONDS.toMinutes(currentTime - timeBought) <= 6
                check
            } else if (getCurrentKeyIap(context).endsWith("5")) {
                TimeUnit.MILLISECONDS.toMinutes(currentTime - timeBought) <= 15
            } else if (getCurrentKeyIap(context).endsWith("10")) {
                TimeUnit.MILLISECONDS.toMinutes(currentTime - timeBought) <= 30
            } else if (getCurrentKeyIap(context).endsWith("20")) {
                true
            } else {
                false
